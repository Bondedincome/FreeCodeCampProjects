...
echo "$($PSQL 'SELECT DISTINCT t.name FROM (SELECT winner_id AS team_id FROM games WHERE year = 2014 AND round = '\''Eighth-Final'\'' UNION SELECT opponent_id FROM games WHERE year = 2014 AND round = '\''Eighth-Final'\'') AS all_teams JOIN teams t ON all_teams.team_id = t.team_id ORDER BY t.name')"
